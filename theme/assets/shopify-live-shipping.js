...
(function() {
  ...
  const ZIP_TO_STATE = {
    ...
    '385': 'TN', '386': 'MS', '387': 'MS', '388': 'MS', '389': 'MS',
  ...
  class ShopifyLiveShipping {
    constructor(options = {}) {
      this.options = {
        defaultCountry: options.defaultCountry || 'United States',
        cacheDuration: options.cacheDuration || 300000, // 5 dakika
        maxRetries: options.maxRetries || 3,
        retryDelay: options.retryDelay || 1000,
        debug: options.debug || false,
        deliveryEstimates: options.deliveryEstimates || {
          'standard': { minDays: 5, maxDays: 8 },
          'express': { minDays: 2, maxDays: 3 },
          'overnight': { minDays: 1, maxDays: 1 },
          'economy': { minDays: 7, maxDays: 14 },
          'ground': { minDays: 3, maxDays: 7 },
          'priority': { minDays: 2, maxDays: 4 },
          'default': { minDays: 5, maxDays: 10 }
        },
        ...options
      };
    ...
    log(...args) {
      if (this.options.debug) {
        console.log('[ShopifyLiveShipping]', ...args);
      }
    }

    /**
     * Prepare rates (start async calculation)
     */
    async prepareRates(shippingAddress) {
      const { zip, country, province } = this.normalizeAddress(shippingAddress);
      
      const url = `/cart/prepare_shipping_rates.json?` + new URLSearchParams({
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(`Prepare rates failed: ${response.status}`);
        }

        return true;
      } catch (error) {
        this.log('Prepare rates error:', error);
        return false;
      }
    }

    /**
     * Fetch calculated rates
     */
    async fetchRates(shippingAddress) {
      const { zip, country, province } = this.normalizeAddress(shippingAddress);
      
      const url = `/cart/async_shipping_rates.json?` + new URLSearchParams({
        'shipping_address[zip]': zip,
        'shipping_address[country]': country,
        'shipping_address[province]': province || ''
      });
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(`Fetch rates failed: ${response.status}`);
        }

        return await response.json();
      } catch (error) {
        this.log('Fetch rates error:', error);
        return null;
      }
    }

    /**
     * Main function: Get shipping rates with polling
     */
    async getShippingRates(shippingAddress, options = {}) {
      const { zip, country, province } = this.normalizeAddress(shippingAddress);
      const cacheKey = `${country}-${province}-${zip}`;
      ...
      if (this.pendingRequests.has(cacheKey)) {
        this.log('Waiting for pending request');
        return this.pendingRequests.get(cacheKey);
      }

      // Start new request
      const requestPromise = this._fetchWithRetry(shippingAddress);
     ...
     */
    async _fetchWithRetry(shippingAddress) {
      // First prepare
      await this.prepareRates(shippingAddress);
      
      // Then poll for results
      for (let attempt = 1; attempt <= this.options.maxRetries; attempt++) {
        this.log(`Fetch attempt ${attempt}/${this.options.maxRetries}`);
        
        // Wait for Shopify to calculate
        await this.delay(this.options.retryDelay * attempt);
        
        const result = await this.fetchRates(shippingAddress);
        
        if (result && result.shipping_rates && result.shipping_rates.length > 0) {
          return this.processRates(result.shipping_rates, shippingAddress);
        }
        
        if (result === null) {
          this.log('Rates not ready yet, retrying...');
        }
      }
    ...
    /**
     * Process and enrich rates
     */
    processRates(rawRates, shippingAddress) {
      const now = new Date();
      
      const processedRates = rawRates.map(rate => {
        const serviceType = this.detectServiceType(rate.name);
        const deliveryEstimate = this.estimateDelivery(rate, serviceType, now);
        const carrier = this.detectCarrier(rate.name, rate.source);
        ...
        return {
          ...
          source: rate.source,
          price: rate.price,
          priceFormatted: this.formatPrice(rate.price, rate.currency),
          ...
          delivery: {
            ...
            minDate: deliveryEstimate.minDate,
            maxDate: deliveryEstimate.maxDate,
            minDateFormatted: this.formatDate(deliveryEstimate.minDate),
            maxDateFormatted: this.formatDate(deliveryEstimate.maxDate),
            rangeText: this.formatDeliveryRange(deliveryEstimate),
            countdown: this.calculateCountdown(deliveryEstimate.minDate)
          },
          isFree: parseFloat(rate.price) === 0,
          isExpress: serviceType === 'express' || serviceType === 'overnight',
          isEconomy: serviceType === 'economy',
          _raw: rate
        };
      });

      // Sort by price
      processedRates.sort((a, b) => parseFloat(a.price) - parseFloat(b.price));

      // Mark cheapest and fastest
      if (processedRates.length > 0) {
        processedRates[0].isCheapest = true;
        
        const fastest = [...processedRates].sort((a, b) => 
          a.delivery.minDays - b.delivery.minDays
        )[0];
        const fastestIndex = processedRates.findIndex(r => r.id === fastest.id);
        if (fastestIndex >= 0) {
          processedRates[fastestIndex].isFastest = true;
        }
      }
      ...
      return {
        ...
        rateCount: processedRates.length,
        rates: processedRates,
        cheapest: processedRates.find(r => r.isCheapest),
        fastest: processedRates.find(r => r.isFastest),
        free: processedRates.filter(r => r.isFree),
        express: processedRates.filter(r => r.isExpress),
        standard: processedRates.filter(r => !r.isExpress && !r.isEconomy),
        economy: processedRates.filter(r => r.isEconomy)
      };
    }
     ...
     */
    detectServiceType(name) {
      const nameLower = name.toLowerCase();
      
      if (nameLower.includes('overnight') || nameLower.includes('next day') || 
          nameLower.includes('1 day') || nameLower.includes('1-day')) {
        return 'overnight';
      }
      
      if (nameLower.includes('express') || nameLower.includes('2 day') || 
          nameLower.includes('2-day') || nameLower.includes('priority') ||
          nameLower.includes('expedited') || nameLower.includes('fast')) {
        return 'express';
      }
      
      if (nameLower.includes('economy') || nameLower.includes('saver') ||
          nameLower.includes('slow') || nameLower.includes('value') ||
          nameLower.includes('budget')) {
        return 'economy';
      }
      
      if (nameLower.includes('ground') || nameLower.includes('standard') ||
          nameLower.includes('regular')) {
        return 'ground';
      }
      
      if (nameLower.includes('priority mail') && !nameLower.includes('express')) {
        return 'priority';
      }
    ...
    /**
     * Detect carrier from name
     */
    detectCarrier(name, source) {
      const nameLower = name.toLowerCase();
      ...
      for (const [carrier, keywords] of Object.entries(carriers)) {
        if (keywords.some(kw => nameLower.includes(kw) || sourceLower.includes(kw))) {
          return {
            code: carrier,
     ...
     */
    estimateDelivery(rate, serviceType, fromDate) {
      let minDays, maxDays;
      
      if (rate.delivery_range) {
        minDays = rate.delivery_range[0] || this.options.deliveryEstimates[serviceType]?.minDays;
        maxDays = rate.delivery_range[1] || this.options.deliveryEstimates[serviceType]?.maxDays;
      } else {
        const estimate = this.options.deliveryEstimates[serviceType] || 
                         this.options.deliveryEstimates.default;
        minDays = estimate.minDays;
        maxDays = estimate.maxDays;
      }

      const minDate = this.addBusinessDays(fromDate, minDays);
      const maxDate = this.addBusinessDays(fromDate, maxDays);
    ...
    calculateCountdown(targetDate) {
      ...
      if (diff <= 0) {
        return { days: 0, hours: 0, minutes: 0, formatted: 'Today' };
      }
      ...
      return { days, hours, minutes, formatted: formatted.trim() || 'Today' };
    }
    ...
    normalizeAddress(address) {
      const zip = address.zip || address.zipCode || address.postal_code || '';
      const country = address.country || address.countryCode || this.options.defaultCountry;
      // KRITIK FIX: Province yoksa ZIP'ten otomatik tespit et
      const province = address.province || address.state || address.region || getStateFromZip(zip);
    ...
    formatPrice(price, currency = 'USD') {
      const numPrice = parseFloat(price) / 100; // Shopify returns cents
    ...
    formatDeliveryRange(estimate) {
      if (estimate.minDays === estimate.maxDays) {
        return this.formatDate(estimate.minDate);
      }
      return `${this.formatDate(estimate.minDate)} - ${this.formatDate(estimate.maxDate)}`;
    }
  ...
  console.log('[ShopifyLiveShipping] v2.0.0 loaded - Province auto-detect fixed');
})();
